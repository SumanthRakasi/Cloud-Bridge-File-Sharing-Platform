...
# S3 details
S3_BUCKET = "sumanthappliedproject"  # e.g., "my-bucket"
S3_REGION = "us-east-2"
...
def upload_file_to_s3(file_bytes, filename):
    """Upload encrypted file bytes to S3. Returns the S3 key if successful."""
    try:
...
st.sidebar.info(
    """
    **CloudBridge** is a secure file sharing solution leveraging AWS S3 for storage and Fernet encryption for file privacy.
    """
)

# --------------------------------------------------------------------------------
# PAGE 1: UPLOAD & ENCRYPT
# --------------------------------------------------------------------------------
def page_upload_encrypt():
    ...
    with st.expander("About CloudBridge"):
        st.markdown(
            ...
            A presigned URL will be generated so you can share the file securely.
            Additionally, the S3 URI of the uploaded object is provided below for alternative access.
            """
        )
    ...
    if uploaded_file is not None and submit_button:
        ...
        time.sleep(1)
        s3_key = upload_file_to_s3(encrypted_bytes, uploaded_file.name)
        ...
        if s3_key:
            ...
            if shareable_link:
                ...
                # Also display the S3 URI of the object
